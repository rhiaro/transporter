...
<html>
  ...
  <body>
    <main>
      ...
      <form role="form" id="list" class="align-center">
        <p>
          <label for="listing">Listing</label>
          <input type="text" name="listing" id="listing"<?=(isset($_GET['listing'])) ? ' value="'.$_GET['listing'].'"' : ''?> />
          <input type="submit" value="Fetch photos" />
        </p>
      </form>

      <form method="post" role="form" id="transporter" class="align-center">

      <?if(isset($_SESSION['listing'])):?>
        <div id="photos-holder">
          <ul>
            <?foreach($_SESSION['listing'] as $photo):?>
              <li><?=$photo?></li>
            <?endforeach?>
          </ul>
        </div>
      <?endif?>
        ...
        <p>
          <label for="target">Album</label>
          <select name="target" id="target">
            <?foreach($albums as $album_uri => $album):?>
              <option value="<?=$album_uri?>"<?=($album_uri==$_POST['target'] || $album_uri==$_GET['listing']) ? " selected": ""?>><?=$album?></option>
            <?endforeach?>
          </select>
        </p>
        ...
        <p>
          ...
          <input type="text" name="name" id="name" value="<?=(isset($_POST['name'])) ? $_POST['name'] : 'Photos'?>" title="This is actually just to set a nice slug... tmp.." />
        </p>
    ...
    <footer class="w1of2 center">
      <p><a href="https://github.com/rhiaro/transporter">Code</a> | <a href="https://github.com/rhiaro/transporter/issues">Issues</a>
      <?if(isset($_SESSION['access_token'])):?>
        | <a href="https://apps.rhiaro.co.uk/transporter?token=<?=$_SESSION['access_token']?>">Quicklink</a>
      <?endif?>
      </p>
    </footer>
    ...
    <script>
      var listingInput = document.getElementById("listing");
      var albumInput = document.getElementById("target");
      albumInput.addEventListener("change", (e) => {
        listingInput.value = e.target.value;
      });
    </script>
  </body>
</html>